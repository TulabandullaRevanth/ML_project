...
async loadOptionsFromAPI(searchTerm = '') {
    ...
    try {
        ...
        console.error('CustomDropdown: Error loading options', error);
    ...
    populateOptions(data) {
        ...
        // Handle array of options or API response with data property
    ...
    loadInitialOptions() {
        if (this.options.loadOptions) {
            // If custom loader function provided
            this.options.loadOptions().then(options => {
                this.populateOptions(options);
            }).catch(error => {
                console.error('CustomDropdown: Error in custom loadOptions:', error);
