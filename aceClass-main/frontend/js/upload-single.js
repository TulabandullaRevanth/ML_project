...
class SingleUploadManager {
    ...
    async loadUserData() {
    console.log('Skipping user auth (no token required)');
    this.updateUserInfo({ name: 'Guest User', plan: 'Free' });
}
    ...
    console.log('Skipping usage stats (no auth required)');
    document.getElementById('worksheets-used').textContent = '0';
    document.getElementById('worksheets-remaining').textContent = '∞';
    document.getElementById('usage-percentage').textContent = '0%';
    ...
    try {
        const studentId = this.studentDropdown?.getValue() || '507f1f77bcf86cd799439012';
        const classId = this.classDropdown?.getValue() || '507f1f77bcf86cd799439013';
        ...
        if (response.ok && result.worksheet && result.worksheet._id) {
            ...
            const redirectUrl = `/pages/grading-split.html?worksheet=${result.worksheet._id}&optimized=true&source=speed-upload`;
